...
root2 = assert(vim.uv.fs_realpath(root2))
...
local real_root = vim.fn.tempname()
vim.fn.mkdir(real_root, 'p')
real_root = assert(vim.uv.fs_realpath(real_root))
local seen = 0
...
-- macOS FSEvents may batch a cold watch for several seconds on a busy runner.
assert(vim.wait(10000, function() return seen > 0 end, 10), 'real fs_event delivered bounded refresh')
real:close()
vim.fn.delete(real_root, 'rf')
print('real fs_event ok')

local race_root = vim.fn.tempname()
vim.fn.mkdir(race_root, 'p')
race_root = assert(vim.uv.fs_realpath(race_root))
