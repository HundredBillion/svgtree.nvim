...
assert(vim.fn.getcwd() == first_root, 'opening a root must change the editor directory')
svgtree.open(second_root)
assert(svgtree.root() == second_root, 'reopened tree must expose its new root')
assert(vim.fn.getcwd() == second_root, 'switching roots must change the editor directory')
svgtree.close()
assert(svgtree.root() == nil, 'closed tree must clear its root')
svgtree.open()
assert(svgtree.root() == second_root, 'opening without a path must keep the last root')
svgtree.close()
